...
There are two requirements that we don't yet have in our schema:
...
The way to do this in Zod is using a [refine](https://zod.dev/?id=refine) function on the schema.
...
Now that we have the schema completed, we can pass it into the `Form` component to render it.
...
We not have a form on the screen! Let's add a theme to make it look nicer.
...
Let's start simple, with a UI schema that will specify the labels of the fields.
...
const uiSchema: FormUiSchema<typeof schema> = {
  /**
    * The attendee field is an array, and array fields render a list of elements.
  ...
  attendee: {
    /**
      * The attendee's element is an object. The UI schema for an object allows
      * specifying the UI for each of its properties.
      * */
    element: {
      email: {
        label: "Email"
...
displayed, if it's `false`, the field will be hidden.
...
<Callout type='info'>
  Note that the type of the data which `cond` takes expects that all data could be `undefined`.
  This is because `cond` is called while the user is filling out the form, and not all data is available yet.
</Callout>

Now try selecting a product, and you'll see that the payment method field appears!
Also, when selecting a debit card, you should see the card number field.
...
of the fields, and the values are the components which render in place of those fields.
  ...
  attendee: {
    element: {
      /**
        ...
        * and the UI schemas for the object's fields.
        * */
      ui: {
        Layout: ({ children }) => (
          <React.Fragment>
